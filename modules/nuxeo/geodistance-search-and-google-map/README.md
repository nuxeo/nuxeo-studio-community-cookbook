...
* `geoloc` schema
* Event handler and its automation chain that fills the `geoloc:location` field
* `geodistance` page provider that uses the elasticsearch `geo_distance` hint
* An operation which returns the Google Map API key

##### Designer
* Import the nuxeo-googlemap-elements
* Import the `nuxeo-geodistance-search.html` element
* And a tab which displays it
* Also add the `geoloc` fields in the layouts

### In your Running Nuxeo Server
* Have a configuration parameter storing the Google Map API Key
* Deploy a Server Template that changes the mapping of elasticsearch, to add the `geo_point` index to the `geoloc:location` field
...
Goal: To fiil ` geoloc:location` by concatenating `geoloc:latitude` and `geoloc:longitude`. Which means this step is optional is you don't use these fields (and modify the ui element), or if you have another way to set `geoloc:location`.
...
#### Add a Configuration Parameter for the Google Map API Key
...
#### Deploy a Configuration Template for Elasticsearch Mapping
